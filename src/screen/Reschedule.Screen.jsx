...
//   const [selectedDate, setSelectedDate] = useState(new Date(selectedSchedule.startTime));
...
//     const selectedIndex = timeSlots.findIndex(item => item.includes(formattedTime));
...
//       try {
//         const updatedSchedule = await updateSchedule(scheduleId, newSchedule); 
//         console.log('Rescheduled Schedule: ', updatedSchedule);
...
//       }
//     } else {
//       Alert.alert('No time slot selected', 'Please select a time slot to reschedule.');
//     }
//   };
...
//           <Text className="text-white text-center text-[20px] font-bold">Reschedule</Text>
//         </TouchableOpacity>
//       </View>
//     </View>
//   );
...
const Reschedule = ({route, navigation}) => {
  const {selectedSchedule,scheduleId} = route.params;
  const [selectedDate, setSelectedDate] = useState(selectedSchedule.startTime);
  const [timeSlots, setTimeSlots] = useState([]);
  const [selectedSlot, setSelectedSlot] = useState(null);
  ...
  useEffect(() => {
    ...
    console.log(formattedTime, selectedIndex);
    setSelectedSlot(selectedIndex);
  }, [timeSlots, selectedSchedule]);
