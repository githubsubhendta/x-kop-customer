...
export const getAllSchedules = async (page=1) => {
  try {
    const apiRes = await appAxios.get(`${BASE_URI}/officer_schedule/schedule-customer`);
...
export const updateSchedule = async (id, updatedData) => {
  try { 
    ...
    const response = await appAxios.put(`${BASE_URI}/officer_schedule/schedules/${id}`, updatedData);
    if (response.data) {
